...
/* 0表示前次Update_Direction_available()回傳的結果是false；1表示前次回傳的結果是true，並會在Update_Direction_available()下一次執行時變回0e。 */
...
static uint8_t Last_Direction = Direction_Undefined;
static inline uint8_t Get_Direction()
{
  /* 結果有更新，因此取得新的移動方向 */
  if(Update_Direction_available()) {
    /*  判斷US1 or US2哪個比較晚結束
     *  三種情況 :
